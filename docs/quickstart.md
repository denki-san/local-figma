...
普通用户从对话开始：把 Figma 目标链接和需求发给 Agent。依赖检查、安装、绑定与排查由 Agent 处理；首次按提示在目标文件导入并运行小插件。无需独立连接窗口，也无需重复填写链接。见 [首次任务提示](first-task.md)。以下命令供 Agent 或主动选择手动操作的开发者查阅。

## 当前选区的高层精修入口

完成链接绑定后，以下入口由 Agent 执行；用户在绑定范围内选择目标并描述修改要求。
...
没有参考图时提供文字视觉方向，进入两种结构草图的比较；有参考图时说明借鉴点和使用权限。Agent 根据对话填写 brief，执行后查看截图并保留待验证项。

## 当前 Alpha 边界

- 当前真实验收来自一个授权文件；多文件兼容性仍需后续验证。
- 当前支持 JavaScript 函数体脚本。执行前检查目标与脚本内容；JSON 作业输入尚未开放。
