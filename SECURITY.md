...
- 可选本地扩展按项目安装和启用，普通命令不加载扩展代码。扩展具有本机 Node.js 权限，只安装已审阅的可信来源；扩展可以按设置说明调用远程服务。
- 可选扩展 API key 单独保存在 `.figma-agent/extension-credentials.json`，文件权限 `0600`，仅在进程内交给启用的扩展。该文件为本机明文；设置响应、普通日志和发行包均不包含密钥。
